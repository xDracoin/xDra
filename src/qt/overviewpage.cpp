...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizexDraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizexDraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizexDraAmount = strAnonymizexDraAmount.remove(strAnonymizexDraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexDraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizexDraAmount*COIN) nMaxToAnonymize = nAnonymizexDraAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizexDraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizexDraAmount));
        strAnonymizexDraAmount = strAnonymizexDraAmount.remove(strAnonymizexDraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexDraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
