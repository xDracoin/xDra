...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizexDraAmount"))
        settings.setValue("nAnonymizexDraAmount", 1000);
    nAnonymizexDraAmount = settings.value("nAnonymizexDraAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizexDraAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizexDraAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizexDraAmount:
            return QVariant(nAnonymizexDraAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizexDraAmount:
            nAnonymizexDraAmount = value.toInt();
            settings.setValue("nAnonymizexDraAmount", nAnonymizexDraAmount);
            emit AnonymizexDraAmountChanged(nAnonymizexDraAmount);
            break;
        default:
            break;
        }
