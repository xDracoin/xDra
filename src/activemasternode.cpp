...
//
// Bootup the masternode, look for a 5000 xDra input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in xDra.conf */
...
{
    CxDracoinAddress address(collateralAddress);
