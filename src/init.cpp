...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: xDra.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: xDrad.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizexDraamount=<n> " + _("Keep N xDra anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. xDra is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. xDra is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("xDra version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "xDra server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of xDra") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart xDra to complete") << "\n";
    ...
    nAnonymizexDraAmount = GetArg("-anonymizexDraamount", 0);
    if(nAnonymizexDraAmount > 999999) nAnonymizexDraAmount = 999999;
    if(nAnonymizexDraAmount < 2) nAnonymizexDraAmount = 2;
    ...
    LogPrintf("Anonymize xDra Amount %d\n", nAnonymizexDraAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1xDra+1000 == (.1xDra+100)*10
       10xDra+10000 == (1xDra+1000)*10
    */
