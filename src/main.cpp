...
unsigned int nStakeMinAge = 12 * 60 * 60; // 1 hour
...
const string strMessageMagic = "xDra Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight > 0 && nHeight <= 1) {
        nSubsidy = 75000 * COIN; // premine 
    }          
    else if (nHeight > 1 && nHeight <= 60) {
        nSubsidy = 1 * COIN;  // premine confirm
    }
    else if (nHeight > 60 && nHeight <= 99) {
        nSubsidy = 1 * COIN; // instamine
    }            
    else if (nHeight > 99  && nHeight <= 199999) {
        nSubsidy = 20 * COIN; // initial block reward
    }
    else if (nHeight > 199999 && nHeight <= 999999) {
        nSubsidy = 15 * COIN;
    }
    else if (nHeight > 999999 && nHeight <= 1999999) {
        nSubsidy = 8 * COIN;
    }
    else if (nHeight > 1999999 && nHeight <= 2000000) {
        nSubsidy = 5000 * COIN; //Last block PoW
    }
    else if (nHeight > 2000000) {
        nSubsidy = 0 * COIN; //End of POW
    }
    return nSubsidy + nFees;
    //Final pow description
}
