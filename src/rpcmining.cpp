...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "xDra is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "xDra is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "xDra is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "xDra is downloading blocks...");
...
Value submitblock(const Array& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIAxDraATION_ERROR, "Block decode failed");
    }
