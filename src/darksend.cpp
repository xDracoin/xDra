...
std::string CDarksendPool::GetStatus()
{
    ...
    switch(state) {
        ...
        case POOL_STATUS_FINAxDraE_TRANSACTION:
...
void CDarksendPool::Check()
{
    ...
    if(fMasterNode) {
        ...
        if(state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions())
        {
            ...
            UpdateState(POOL_STATUS_FINAxDraE_TRANSACTION);
        }
    }

    // create the finalized transaction for distribution to the clients
    if(state == POOL_STATUS_FINAxDraE_TRANSACTION) {
        LogPrint("darksend", "CDarksendPool::Check() -- FINAxDraE TRANSACTIONS\n");
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat xDra and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002xDra per transaction on average.
            */
            if(r <= 10)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizexDraAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePos, strFail, coinControl, ONLY_NONDENOMINATED_NOT5000IFMN);
    if(!success){
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, nChangePos, strFail, coinControl, ONLY_NOT5000IFMN);
        if(!success){
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100xDra+1 ( bit on if present )
    // bit 1 - 10xDra+1
    // bit 2 - 1xDra+1
    // bit 3 - .1xDra+1
    // bit 3 - non-denom
